        ...
        if(isset($_POST['banner_nome'])) {
            ...
            if($json['error']) {
                ...
                for($k=0; $k<count($banner_nome); $k++) {
                    move_uploaded_file($banner_path_tmp[$k], "__system__/img/Banner_TCC/{$banner_path[$k]}");
            ...
            if($json['error']) {
                ...
                if(!empty($banner_path)) {
                    move_uploaded_file($banner_path_tmp, "__system__/img/Banner_TCC/$banner_path");
                    $new_img = TRUE;
                }
            ...
            $sel = $conn->prepare("SELECT * FROM banner LIMIT $begin, $qtd_result");
            $sel->execute();
            if($sel) {
                if($sel->rowCount() > 0) {
                    $json['empty'] = FALSE;
